...
import argparse
output_dir = 'SFML_Outputs2_LastDay'
...
if __name__ == '__main__':
    ...
    parser.add_argument(
       ...
       help="training, testing, transfer_learning"
    )
  ...
  #OPTION 2 HDF5 stackoverflow get specific key instead of loading each of them
    ...
    if args.mode == "training":   
      ...
      history = model.fit(
          ...
          steps_per_epoch=math.floor(len(train_dataframe)/args.batch_size),
          #steps_per_epoch=(len(train_dataframe)//(args.batch_size * args.epoch)),
          validation_data=validation_dataset.repeat(),
          validation_steps=math.floor(len(validation_dataframe)/args.batch_size),
          #validation_steps=(len(validation_dataframe)//(args.batch_size * args.epoch)),
          epochs=args.epoch,
      ...
      if os.path.isfile(checkpoint) != False:
        ...
        print(f'Overall: accuracy={acc:.4f}, precision={prec:.4f}, recall={rec:.4f}, ' \
              f'specificity={spec:.4f}, f1={f1:.4f}, mcc={mcc:.4f}, auroc={auc:.4f}, auprc={prc:.4f}')
      
        # 1) Individual scores for each protein
        test_dataframe['protein'] = test_dataframe['p2'].str.split('_').str[0]
        group_metrics = []
        for gp, df_grp in test_dataframe.groupby('protein'):
            idx = df_grp.index.values
            y_true_gp = y_true[idx]
            y_pred_gp = y_pred[idx]
            y_pred_round = np.round(y_pred_gp).astype(int)
            cm = confusion_matrix(y_true_gp, y_pred_round, labels=[0,1])
            acc_gp = (cm[0,0] + cm[1,1]) / cm.sum() if cm.sum()>0 else float('nan')
            prec_gp = precision_score(y_true_gp, y_pred_round, zero_division=0)
            rec_gp = recall_score(y_true_gp, y_pred_round, zero_division=0)
            f1_gp = 2 * (prec_gp * rec_gp) / (prec_gp + rec_gp) if (prec_gp+rec_gp)>0 else float('nan')
            mcc_gp = matthews_corrcoef(y_true_gp, y_pred_round) if len(np.unique(y_true_gp))>1 else float('nan')
            try:
                auc_gp = metrics.roc_auc_score(y_true_gp, y_pred_gp)
            except ValueError:
                auc_gp = float('nan')
            prc_gp = metrics.average_precision_score(y_true_gp, y_pred_gp)
        ...
        metrics_df = pd.DataFrame(group_metrics).set_index('protein')
        print("\nPer protein performance:")
        print(metrics_df)

        # 2) Plot success rates (accuracy) for each protein
        plt.figure(figsize=(10,6))
        metrics_df['accuracy'].sort_values().plot(kind='bar')
        plt.ylabel('Accuracy')
        plt.xlabel('Protein')
        plt.title('Success Rate (Accuracy) per protein')
      ...
      if os.path.isfile(checkpoint) != False:
        model = tf.keras.models.load_model(checkpoint)
         ...
         print("you have to pass the model for testing")    
      ...
      # 1) Individual scores for each G-protein
      # assume G-protein identifier is prefix of 'p2' before first underscore
      test_dataframe['protein'] = test_dataframe['p2'].str.split('_').str[0]
      group_metrics = []
      for gp, df_grp in test_dataframe.groupby('protein'):
          idx = df_grp.index.values
      ...
      plt.ylabel('F1-Score')
      plt.xlabel('Protein')
      plt.title('F1-score per protein')
      plt.xticks(rotation=45, ha='right')
      plt.tight_layout()
      fname = "f1.png"
      ...
      f1_col = 'f1'
      order = metrics_df['accuracy'].sort_values().index
      to_plot = metrics_df.loc[order, ['accuracy', f1_col]].rename(columns={f1_col: 'f1'})

      ax = to_plot.plot(kind='bar')  # grouped bars: accuracy & f1 side-by-side
      plt.ylabel('Score')
      plt.xlabel('Protein')
      plt.title('Accuracy & F1 per Protein')
      plt.xticks(rotation=45, ha='right')
